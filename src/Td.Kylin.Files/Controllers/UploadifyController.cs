...
namespace Td.Kylin.Files.Controllers
{
    ...
    public class UploadifyController : Controller
    {
        ...
        /// <returns></returns>
        [HttpPost("data")]
        public IActionResult Upload(string data, string savepath, bool fixedpath = true, string name = null, string extension = null, bool beOverride = false, long compressIfGreaterSize = 0, int maxWidth = 0, int maxHeight = 0, bool cutIfOut = false)
        {
            UploadResult result = new UploadResult();

            if (string.IsNullOrWhiteSpace(data))
            {
                result.Message = "文件数据不正常";
                return Json(result);
            }
            ...
            {
                result.Message = "文件数据异常，非有效的Base64数据";
                return Json(result);
            }
